	...
	buscador_modelo = buscador_modelo.replace(/ /g, "-");
			...
			jQuery.ajax({
				...
				success:function(data){
					...
					window.location.href = URL_BUSQUEDA+'?s=&post_type=product';
				},
				error: function ( xhr, errorType, exception ) { //Triggered if an error communicating with server
		            var errorMessage = exception || xhr.statusText; //If exception null, then default to xhr.statusText
		            console.log(xhr);
...
/*
function getIDs(marca){//Metodo que obtiene el label con el rowid de la marca para concatenar a la url
	var buscador_marca_label = marca;
			var data = {
				marca_label: buscador_marca_label,
				action : 'actualizar'
